...
\marginpar{atrent: da qualche parte descrivi anche il contesto in cui hai lavorato, serve per capire che le richieste funzionali che ti sono state fatte erano adeguate}

\marginpar{ac: in che senso? di cosa dovrei parlare e dove potrei mettere quella sezione?}
...
\begin{figure}
	\center{\includegraphics[width=0.5\linewidth]{./img/receiver_transmitter_module.jpg}}
	\caption{Modulo transceiver MX-05V/XD-RF-5V}
	\label{fig:rx_tx_module}
\end{figure}

\begin{figure}
	\center{\includegraphics[width=0.5\linewidth]{./img/clonare_telecomando.png}}
...
Controllare un servomotore con l'Arduino è in realtà semplice perché esiste una libreria (\url{https://www.arduino.cc/en/reference/servo}) che ne facilita il lavoro: basta pensare in gradi (esempio di collegamento tra servo e Arduino nella Figura \ref{fig:servo_uno} e di codice nella Figura \ref{fig:servo_code}).
...
\begin{figure}
	\center{\includegraphics[width=0.5\linewidth]{./img/esp01_flashed_code.png}}
...
\begin{figure}
	\center{\includegraphics[width=0.5\linewidth]{./img/esp01_listen.png}}
...
\begin{figure}
	\center{\includegraphics[width=0.6\linewidth]{./img/arduino_esp_serial_setup.png}}
...
Come si è visto nelle Figure \ref{fig:esp_check} e \ref{fig:esp_log}, l'Arduino inoltra all'ESP-01 delle stringhe che sono quasi in formato JSON: prima dell'oggetto JSON c'è un'altra stringa usata per identificare la funzione voluta (e.g$.$ "check\{object\}" e "log\{object\}"). L'ESP-01 poi filtra questi dettagli non più necessari (come nella Figura \ref{fig:esp01_listen}) e chiama la funzione richiesta. Viene presentato un esempio che descrive la sequenza degli eventi per la verifica del tag:
...
\begin{figure}
	\center{\includegraphics[width=0.75\linewidth]{./img/esempio_libro_soci.png}}
...
\end{lstlisting}
	...
	\begin{itemize}
		\item definire delle middleware per rispondere a richieste HTTP e per gestire gli errori. Esempio di middleware per renderizzare la pagina "404.ejs" nel caso in cui un utente provi ad accedere ad una pagina non esistente:
\begin{lstlisting}
...
\end{lstlisting}
		Quando viene fatta una richiesta GET all'endpoint "/" allora, come risposta, l'applicazione invia all'utente un file chiamato "index.ejs" (che rappresenta la homepage) che viene poi renderizzato dal suo browser;
		\item renderizzare pagine HTML in maniera dinamica passando degli argomenti ai template (e.g. usando un linguaggio di templating chiamato EJS);
...
\end{lstlisting}
	...
	\item Mongoose (\url{https://mongoosejs.com/}) è una libreria di ODM (object data modeling) che fornisce un ambiente per la modelizzazione dei dati di MongoDB permettendo la definizione degli oggetti sotto uno schema fortemente tipizzato (si veda la sezione \ref{sec:db} "Database")
	
\end{itemize}

\begin{figure}
	\center{\includegraphics[width=0.45\linewidth]{./img/node_modules.png}}
	\caption{I moduli di Node.js utilizzati dall'applicazione}
	\label{fig:node_modules}
\end{figure}
...
\begin{itemize}
	\item Il modello corrisponde alla struttura e alla logica dei dati dell'applicazione (esempio nella Figura \ref{fig:log_model}).
	\item La vista corrisponde alla logica dell'UI (User Interface) dell'applicazione. Rappresenta il modo in cui vengono presentati i dati all'utente (esempio nella Figura \ref{fig:tessere_view}). 
	\item Il controller accetta l'input dell'utente (e.g. visitare il sito web, cliccare su un pulsante, inviare un modulo ecc.) e lo trasforma in comandi per il modello o per la vista (esempio nella Figura \ref{fig:soci_controller}).
\end{itemize}

\begin{figure}
	\center{\includegraphics[width=1\linewidth]{./img/log_model.png}}
	\caption{Esempio di codice del modello creato per i log}
	\label{fig:log_model}
\end{figure}

\begin{figure}
	\center{\includegraphics[width=1\linewidth]{./img/tessere_view.png}}
	\caption{Esempio di una parte del codice HTML della vista creata per presentare le tessere all'utente}
	\label{fig:tessere_view}
\end{figure}

\begin{figure}
	\center{\includegraphics[width=1\linewidth]{./img/soci_controller.png}}
	\caption{Esempio di una parte del codice del controller creato per gestire i comandi relativi ai soci. Questa funzione, quando viene chiamata, cerca nel database i soci in base al termine inviato tra i parametri e poi renderizza una pagina con i risultati.}
	\label{fig:soci_controller}
\end{figure}
