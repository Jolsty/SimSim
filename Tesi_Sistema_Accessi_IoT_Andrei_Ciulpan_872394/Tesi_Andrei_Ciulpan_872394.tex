...
\begin{lstlisting}[caption={Esempio di codice per controllare il servomotore: inanzittutto il servo viene collegato al pin 9 dell'Arduino e viene fatto ruotare di 180 gradi da una parte; dopo 5 secondi viene fatto ruotare di 180 gradi dalla parte opposta. }, label={lst:servo}]
#include <Servo.h>
...
\begin{lstlisting}[caption={L'Arduino manda all'ESP-01 il seriale della tessera in formato JSON e aspetta la risposta con lo status code. Se statusCode = 200 allora mette la variabile allowed a \textquotedblleft{} true \textquotedblright{} (questo pezzo non è presentato per motivi di lunghezza). La funzione printBodyToESP si può vedere nel Listato \ref{lst:uno_printBodyToESP} }, label={lst:uno_checkCardSerial}]
...
\begin{lstlisting}[caption={L'ESP-01 manda una richiesta POST all'endpoint specificato. Il corpo della richiesta è in formato JSON e include il seriale e il tipo della tessera che si vuole verificare.}, label={lst:esp_checkTessera}]
...
\begin{lstlisting}[caption={L'ESP-01 manda una richiesta POST all'endpoint specificato. Il corpo della richiesta è in formato JSON e rappresenta il log.}, label={lst:esp_sendLog}]
...
\end{lstlisting}
Un esempio di utilizzo di SoftwareSerial si può trovare nel Listato (\ref{lst:uno_printBodyToESP}).
...
\begin{lstlisting}[language=HTML, morekeywords={if, else}]
...
\begin{lstlisting}[morekeywords={ @media, max-width, px, width, height, !important}]
...
\begin{lstlisting}[language=JavaScript, morekeywords={ alert }]
function myFunction() {
...
\begin{lstlisting}[language=JavaScript, morekeywords={ confirm, $, alert }]
