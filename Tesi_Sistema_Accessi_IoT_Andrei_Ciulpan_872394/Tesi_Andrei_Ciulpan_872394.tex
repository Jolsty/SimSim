...
\end{lstlisting}
	 Questa funzione, quando viene chiamata, cerca nel database i soci in base al termine inviato tra i parametri e poi renderizza una pagina con i risultati.
\end{itemize}
...
\begin{itemize}
	...
	\item query più semplificate e leggibili grazie al "method chaining" di JavaScript (soprattutto quando diventano complesse). Esempio:
\begin{lstlisting}
const query = Account.find({})
.where('nome').equals('Andrea');
query.exec();
\end{lstlisting}
