...
exports.fullTableSearch = (req, res) => {
	...
	Log.find( { $text: { $search: searchTerm }})
		.exec(function(err, logs) {
			...
			} else return res.render("../views/logs/logs_index", { searched: "true", logs: logs });
		});
}
	...
	switch (date.length) {
		...
		case 1:
			...
			Log.find( { data: { "$gte": new Date(year, 0, 1, 0, 0, 0), "$lte": new Date(year, 11, 31, 23, 59, 59) } }, (err, logs) => {
				if (err) console.error(err);
				else return res.render("../views/logs/logs_index", { searched: "true", data: req.params.data, logs: logs });
			});
			break;
