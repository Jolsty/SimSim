...
exports.viewAllSoci = (req, res) => {
	...
	Socio.find( (err, soci) => {
		...
		} else return res.render("../views/soci/soci_index", { soci: soci, sociString: JSON.stringify(soci) });
	});
};
...
exports.viewSocioByNumeroTessera = (req, res) => {
	...
	Socio.
		...
		exec( (err, socio) => {
			...
			if (socio) return res.render("../views/soci/soci_index", { soci: socio, sociString: JSON.stringify(socio) });
			else return res.send("Non esiste un socio con quel numero tessera");
		});
};

exports.viewSociByNomeCognome = (req, res) => { // rimuovere?
