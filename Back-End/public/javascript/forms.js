...
// ricercaMultiplaSociForm

function ricercaMultiplaSoci() {
    let url = indirizzo;
    let form = $('#ricercaMultiplaSociForm');
    let cercaTesto = $('#cercaSoci')[0].value;
    let termine = $("#ricercaSelect")[0].value;

    switch (termine) {

        case "nome_e_cognome":

            cercaTesto = cercaTesto.split(" ");
            if (cercaTesto.length !== 2) return alert("Inserisci SOLAMENTE il nome e il cognome");
            let nome = cercaTesto[0];
            let cognome = cercaTesto[1];
            url = url + "/api/soci/viewByNomeCognome/" + nome + "/" + cognome;
            break;

        case "numero_tessera":

            let numero_tessera = cercaTesto;
            url = url + "/api/soci/viewByNumeroTessera/" + numero_tessera;
            break;

        case "ruolo":

            let ruolo = cercaTesto;
            url = url + "/api/soci/viewByRuolo/" + ruolo;
            break;

        case "regione":

            let regione = cercaTesto;
            url = url + "/api/soci/viewByRegione/" + regione;
            break;

        case "provincia":

            let provincia = cercaTesto;
            url = url + "/api/soci/viewByProvincia/" + provincia;
            break;

        case "citta":

            let citta = cercaTesto;
            url = url + "/api/soci/viewByCitta/" + citta;
            break;
    }
